...
(function(){
  ...
  wFinish?.addEventListener('click', ()=>{
    ...
    // Gather values
    const name = (currentUser && currentUser.name) ? currentUser.name : ((nickInput?.value || '').trim());
    ...
    // Open card in new tab with params
    // Try to grab avatar preview dataURL inline for query param
    let avatarData = '';
    try{
      const bg = (getComputedStyle(avatarPreview||document.createElement('div')).backgroundImage||'')
        .replace(/^url\(["']?/, '').replace(/["']?\)$/, '');
      if (bg && /^data:image\//.test(bg)) avatarData = bg;
    }catch(_){}
    const params = new URLSearchParams({ name, role, tri:String(trillions), joined, stable:String(stable), score:String(score), tier, avatar: (window.__plasmaAvatarData || avatarData) });
    const url = 'card.html?_v=pt11&' + params.toString();
